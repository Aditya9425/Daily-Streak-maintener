-- AI coaching tables for Gemini-powered features

-- Daily AI coaching messages
CREATE TABLE IF NOT EXISTS public.ai_daily_coaching (
...
CREATE TABLE IF NOT EXISTS public.ai_missed_task_feedback (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE NOT NULL,
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_ai_daily_coaching_user_date ON public.ai_daily_coaching(user_id, date);
CREATE INDEX IF NOT EXISTS idx_ai_missed_feedback_user_task ON public.ai_missed_task_feedback(user_id, task_id);
CREATE INDEX IF NOT EXISTS idx_ai_weekly_reviews_user_week ON public.ai_weekly_reviews(user_id, week_start_date);